...
//    //        else
//    //        {
//    //            if (Vector3.Distance(transform.position, GameObject.FindGameObjectWithTag("newplayer").transform.position) >= 2.5f)
...
//            // Calculate the direction towards the player with the offset distance
//            //Vector3 targetPosition = m_PlayerPosition + (transform.position - m_PlayerPosition).normalized * offsetDistance;
//            // Calculate the direction towards the player with the offset distance
//            Vector3 targetPosition = m_PlayerPosition + (new Vector3(transform.position.x, 0, transform.position.z) - new Vector3(m_PlayerPosition.x, 0, m_PlayerPosition.z)).normalized * offsetDistance;


//            navMeshAgent.SetDestination(targetPosition); // Set the destination of the enemy to the adjusted target position
//        }

//        if (navMeshAgent.remainingDistance <= navMeshAgent.stoppingDistance)
//        {
//            if (m_WaitTime <= 0 && !m_CaughtPlayer && Vector3.Distance(transform.position, GameObject.FindGameObjectWithTag("newplayer").transform.position) >= 6f)
//            {
//                // Check if the enemy is not near to the player, returns to patrol after the wait time delay
//                m_IsPatrol = true;
//                m_PlayerNear = false;
...
public class nav : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        // Disable auto-updating position by setting update position to false
